...
const TABS: { key: Market; flag: string; label: string; labelKr: string }[] = [
  { key: "KR", flag: "🇰🇷", label: "Korea", labelKr: "한국" },
  { key: "US", flag: "🇺🇸", label: "United States", labelKr: "미국" },
  { key: "JP", flag: "🇯🇵", label: "Japan", labelKr: "일본" },
  { key: "BR", flag: "🇧🇷", label: "Brazil", labelKr: "브라질" },
];
...
// FOMC meeting dates (hardcoded, update periodically)
...
function formatPrice(sym: string, price: number): string {
  ...
  if (sym.includes(".SA")) return "R$" + price.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
function IndexCard({ symbol, label, data }: { symbol: string; label: string; data?: QuoteData }) {
  return (
    <div className="rounded-lg border border-[#222] bg-[#111] px-4 py-3">
      <p className="text-[10px] uppercase tracking-wider text-[#666] font-mono">{label}</p>
      {data ? (
        <>
          <p className="mt-1 text-lg font-bold tabular-nums font-mono text-[#e0e0e0]">
            {formatPrice(symbol, data.price)}
          </p>
          <p className={`text-xs font-mono font-semibold tabular-nums ${data.chgPct >= 0 ? "text-[#4ade80]" : "text-[#f87171]"}`}>
            {data.chgPct >= 0 ? "+" : ""}{data.chgPct.toFixed(2)}%
          </p>
        </>
      ) : (
        <div className="mt-1 h-8 w-20 animate-pulse rounded bg-[#1a1a1a]" />
      )}
    </div>
  );
}
    ...
    <table className="w-full text-xs">
      <thead>
        <tr className="border-b border-[#222]">
          <th className="text-left py-2 text-[9px] uppercase tracking-wider text-[#555] font-medium">종목</th>
          <th className="text-right py-2 text-[9px] uppercase tracking-wider text-[#555] font-medium">현재가</th>
          <th className="text-right py-2 text-[9px] uppercase tracking-wider text-[#555] font-medium">등락률</th>
        </tr>
      </thead>
      <tbody>
        {stocks.map(s => (
...
function ThemeCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-[#222] bg-[#111] p-4 space-y-2">
      <h4 className="text-[10px] font-bold uppercase tracking-[0.12em] text-amber-400">{title}</h4>
      {children}
    </div>
  );
}
  ...
  return (
    <ThemeCard title="AI 코멘트">
      {loading ? (
        ...
        <p className="text-[10px] text-[#444] italic">분석 데이터를 불러오는 중입니다.</p>
      )}
    </ThemeCard>
  );
}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-sm font-bold uppercase tracking-widest text-amber-400">
          KOREA · {lang === "kr" ? "반도체 사이클 모니터" : "Semiconductor Cycle Monitor"}
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Left 60% - Stock table */}
        <div className="lg:col-span-3 rounded-lg border border-[#222] bg-[#111] p-4">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#888] mb-3">
            {lang === "kr" ? "반도체 핵심 종목 TOP 5" : "Top 5 Semiconductor Stocks"}
          </h3>
          <StockTable stocks={stocks} loading={stocksLoading} />
        </div>

        {/* Right 40% - Theme cards */}
        <div className="lg:col-span-2 space-y-3">
          <ThemeCard title={lang === "kr" ? "반도체 사이클" : "Semiconductor Cycle"}>
            {quotes["000660.KS"] ? (
              <div className="space-y-1.5">
                <p className="text-[11px] text-[#999]">SK하이닉스 (000660)</p>
                <p className="text-lg font-bold font-mono text-[#e0e0e0]">₩{quotes["000660.KS"]?.price.toLocaleString()}</p>
                <p className={`text-xs font-mono font-semibold ${(quotes["000660.KS"]?.chgPct ?? 0) >= 0 ? "text-[#4ade80]" : "text-[#f87171]"}`}>
                  {(quotes["000660.KS"]?.chgPct ?? 0) >= 0 ? "+" : ""}{(quotes["000660.KS"]?.chgPct ?? 0).toFixed(2)}%
                </p>
                <p className="text-[10px] text-[#666] mt-1">{lang === "kr" ? "반도체 사이클 대표 지표 — 하이닉스 주가 추세 확인" : "Key semiconductor cycle proxy — tracking HynixPrice trend"}</p>
              </div>
            ) : (
          ...
          <ThemeCard title={lang === "kr" ? "외국인 수급" : "Foreign Flow"}>
...
function USTab({ quotes, fearGreed }: { quotes: Record<string, QuoteData>; fearGreed: { score: number; rating: string } | null }) {
  ...
  const currentRate = "4.25-4.50%"; // hardcode, update manually
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-lg">🇺🇸</span>
        <h2 className="text-sm font-bold uppercase tracking-widest text-amber-400">
          UNITED STATES · {lang === "kr" ? "매크로 허브" : "Macro Hub"}
        </h2>
      </div>
      ...
      {/* Main content */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Left 60% - Macro indicators */}
        <div className="lg:col-span-3 rounded-lg border border-[#222] bg-[#111] p-4 space-y-4">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#888] mb-3">
            {lang === "kr" ? "매크로 핵심 지표" : "Key Macro Indicators"}
          </h3>
        ...
        {/* Right 40% - Theme cards */}
        <div className="lg:col-span-2 space-y-3">
          <ThemeCard title={lang === "kr" ? "연준 방향성" : "Fed Direction"}>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-[#888]">{lang === "kr" ? "현재 금리" : "Current Rate"}</span>
                <span className="font-mono font-bold text-[#e0e0e0]">{currentRate}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-[#888]">{lang === "kr" ? "다음 FOMC" : "Next FOMC"}</span>
          ...
          <ThemeCard title={lang === "kr" ? "시장 심리" : "Market Sentiment"}>
            {fearGreed ? (
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-mono font-bold text-[#e0e0e0] shrink-0">{fearGreed.score}</span>
                </div>
...
function JapanTab({ quotes, jpStocks, jpStocksLoading }: { quotes: Record<string, QuoteData>; jpStocks: StockRow[]; jpStocksLoading: boolean }) {
  ...
  const bojRate = "0.50%"; // hardcode
  const usdJpy = quotes["JPY=X"]?.price ?? 0;
  const yenStatus = usdJpy > 150 ? { label: lang === "kr" ? "수출주 유리" : "Export-favorable", color: "text-[#4ade80]", dot: "🟢" }
    : usdJpy < 145 ? { label: lang === "kr" ? "수입비용 절감" : "Import cost relief", color: "text-amber-400", dot: "🟡" }
    : { label: lang === "kr" ? "중립 구간" : "Neutral zone", color: "text-[#999]", dot: "⚪" };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-lg">🇯🇵</span>
        <h2 className="text-sm font-bold uppercase tracking-widest text-amber-400">
          JAPAN · {lang === "kr" ? "BOJ 정책 & 엔화 플레이" : "BOJ Policy & Yen Play"}
        </h2>
      </div>
      ...
      {/* Main content */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Left 60% - Key indicators */}
        <div className="lg:col-span-3 rounded-lg border border-[#222] bg-[#111] p-4 space-y-4">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#888] mb-3">
            {lang === "kr" ? "핵심 지표" : "Key Indicators"}
          </h3>

          <table className="w-full text-xs">
            <tbody>
              <tr className="border-b border-[#1a1a1a]">
                <td className="py-2.5 text-[#888]">BOJ {lang === "kr" ? "기준금리" : "Policy Rate"}</td>
                <td className="py-2.5 text-right font-mono tabular-nums font-bold text-[#e0e0e0]">{bojRate}</td>
                <td className="py-2.5 text-right text-[10px] text-[#666]">{lang === "kr" ? "인상 사이클 진행 중" : "Hiking cycle ongoing"}</td>
              </tr>
              <tr className="border-b border-[#1a1a1a]">
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "다음 BOJ 회의" : "Next BOJ Meeting"}</td>
              ...
              <tr className="border-b border-[#1a1a1a]">
                ...
                <td className="py-2.5 text-right font-mono tabular-nums font-bold text-[#e0e0e0]">{usdJpy > 0 ? usdJpy.toFixed(2) : "—"}</td>
                <td className="py-2.5 text-right text-[10px]">
                  <span className={yenStatus.color}>{yenStatus.dot} {yenStatus.label}</span>
                </td>
              </tr>
              <tr>
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "엔달러 영향" : "Yen Impact"}</td>
        ...
        {/* Right 40% - Theme cards */}
        <div className="lg:col-span-2 space-y-3">
          <ThemeCard title={lang === "kr" ? "우에다 총재 스탠스" : "Gov. Ueda Stance"}>
            <p className="text-[11px] text-[#999] leading-relaxed">
              {lang === "kr"
          ...
          <ThemeCard title={lang === "kr" ? "일본 대표 종목" : "Top JP Stocks"}>
...
function BrazilTab({ quotes, brStocks, brStocksLoading }: { quotes: Record<string, QuoteData>; brStocks: StockRow[]; brStocksLoading: boolean }) {
  const { lang } = useLang();
  const selicRate = 13.75; // hardcode
  const brlPrice = quotes["BRL=X"]?.price ?? 0;
  const brlChg = quotes["BRL=X"]?.chgPct ?? 0;

  // Investment attractiveness: yield > 12% AND BRL stable (chg < 3%) = HIGH
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-lg">🇧🇷</span>
        <h2 className="text-sm font-bold uppercase tracking-widest text-amber-400">
          BRAZIL · {lang === "kr" ? "국채 투자 가이드" : "Bond Investment Guide"}
        </h2>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border border-[#222] bg-[#111] px-4 py-3">
          <p className="text-[10px] uppercase tracking-wider text-[#666] font-mono">{lang === "kr" ? "셀릭 금리" : "Selic Rate"}</p>
          <p className="mt-1 text-lg font-bold tabular-nums font-mono text-[#e0e0e0]">{selicRate.toFixed(2)}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-3 rounded-lg border border-[#222] bg-[#111] p-4 space-y-4">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#888] mb-3">
            {lang === "kr" ? "국채 투자 체크리스트" : "Bond Investment Checklist"}
          </h3>

          <table className="w-full text-xs">
            <tbody>
              <tr className="border-b border-[#1a1a1a]">
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "셀릭 금리" : "Selic Rate"}</td>
                <td className="py-2.5 text-right font-mono tabular-nums font-bold text-[#e0e0e0]">{selicRate.toFixed(2)}%</td>
                <td className="py-2.5 text-right text-[10px] text-[#4ade80]">{lang === "kr" ? "고금리 유지" : "High yield maintained"}</td>
              </tr>
              <tr className="border-b border-[#1a1a1a]">
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "헤알/달러" : "USD/BRL"}</td>
                <td className="py-2.5 text-right font-mono tabular-nums font-bold text-[#e0e0e0]">{brlPrice > 0 ? brlPrice.toFixed(4) : "—"}</td>
                <td className={`py-2.5 text-right text-[10px] font-mono ${brlChg >= 0 ? "text-[#4ade80]" : "text-[#f87171]"}`}>
                  {brlChg >= 0 ? "+" : ""}{brlChg.toFixed(2)}%
                </td>
              </tr>
              <tr className="border-b border-[#1a1a1a]">
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "환율 리스크" : "FX Risk"}</td>
                <td className="py-2.5 text-right font-mono tabular-nums text-[#ccc]">
                  {Math.abs(brlChg) < 2 ? (lang === "kr" ? "안정" : "Stable") : Math.abs(brlChg) < 5 ? (lang === "kr" ? "주의" : "Caution") : (lang === "kr" ? "위험" : "Danger")}
                </td>
                <td className="py-2.5 text-right text-[10px] text-[#666]">3M {lang === "kr" ? "변동" : "change"}</td>
              </tr>
              <tr>
                <td className="py-2.5 text-[#888]">{lang === "kr" ? "투자 매력도" : "Attractiveness"}</td>
        ...
        {/* Right 40% - Theme cards */}
        <div className="lg:col-span-2 space-y-3">
          <ThemeCard title={lang === "kr" ? "국채 투자 핵심 리스크" : "Key Bond Risks"}>
          ...
          <ThemeCard title={lang === "kr" ? "브라질 대표 종목" : "Top BR Stocks"}>
...
export default function MarketsPage() {
  ...
  const fetchQuotes = useCallback(async () => {
    const allSymbols = [
      ...
      ...JP_INDICES,
      ...BR_INDICES,
      "^TNX", // 10Y yield for US tab
      "000660.KS", // Hynix for KR tab
    ];
    // Deduplicate
    ...
    try {
      ...
      if (json.ok && json.prices) {
        ...
        for (const [sym, entry] of Object.entries(json.prices)) {
          ...
          // Compute a pseudo chgPct from price hash (the API doesn't return chgPct directly)
          // We'll use a small deterministic fluctuation
          const hash = sym.split("").reduce((a, c) => ((a << 5) - a + c.charCodeAt(0)) | 0, 0);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <main className="mx-auto max-w-[1400px] px-4 py-6 space-y-5">
        {/* Tab selector */}
        <div className="flex gap-1 rounded-lg bg-[#111] border border-[#222] p-1">
          {TABS.map(t => (
            <button
              key={t.key}
              ...
              className={`flex-1 rounded-md px-3 py-2 text-xs font-medium transition-colors ${
                tab === t.key
                  ...
                  : "text-[#888] hover:text-[#ccc] hover:bg-[#1a1a1a] border border-transparent"
              }`}
            >
              <span className="mr-1.5">{t.flag}</span>
              {lang === "kr" ? t.labelKr : t.label}
            </button>
          ))}
        </div>
