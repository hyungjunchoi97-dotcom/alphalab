  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-px mb-px">
        {WEEKDAYS.map((wd) => (
          <div key={wd} className="py-0.5 text-center text-[8px] font-mono font-bold tracking-widest text-[#777]">
            {wd}
          </div>
        ))}
      </div>
        ...
        {days.map(({ date, key, inMonth }) => {
          ...
          return (
            <div
              ...
              className={`relative min-h-[48px] p-1 border ${cellBorder} ${cellBg} transition-colors ${
                inMonth ? "" : "opacity-30"
            ...
            >
              ...
              <span
                className={`block text-[9px] font-mono leading-tight ${
                  isToday ? "text-amber-400 font-bold" : "text-[#999]"
                } ${date.getDay() === 0 ? "text-red-400/90" : ""} ${date.getDay() === 6 ? "text-blue-400/90" : ""}`}
              >
                {date.getDate()}
              </span>
